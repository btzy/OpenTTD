...
void BaseVehicleListWindow::BuildVehicleList()
{
	...
	if (this->grouping == GB_NONE) {
		...
		while (begin != this->vehicles.end()) {
			const Vehicle * const first_shared = (*begin)->FirstShared();
			VehicleList::const_iterator end = std::find_if_not(begin, this->vehicles.cend(), [first_shared](const Vehicle * const &v) {
